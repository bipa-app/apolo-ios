<h1>Apolo <img src="https://github.com/user-attachments/assets/04006e9f-8199-4d71-997f-5a3324add979" align="right" width="74"/></h1>
<p>
  <img src="https://img.shields.io/badge/Swift-orange.svg" />
  <img src="https://img.shields.io/badge/version-0.0.1-blue.svg" />
</p>

Apolo is Bipa's Design System, a set of guidelines that help our teams build our products consistently and efficiently. This design system provides a robust foundation of components and patterns that ensure a cohesive user experience across all Bipa's current and future applications.

## Components Checklist

### Foundation
- [x] Colors
  - [x] Core brand colors (Blue, Violet, Red, Orange, Yellow, Green, Aqua, Pink)
  - [x] Core neutral colors (Black, White, Gray scales)
  - [x] Light/Dark variants

### Typography
- [x] Font Family
  - [x] ABC Ginto (Regular, Medium, Bold)
- [x] Text Styles
  - [x] SuperLargeTitle
  - [x] ExtraLargeTitle
  - [x] ExtraLargeTitle2
  - [x] Title1-3
  - [x] Headline
  - [x] Body
  - [x] Callout
  - [x] Subheadline
  - [x] Footnote
  - [x] Caption1-2

### Iconography
- [x] SF Symbols
- [x] Noto Emoji
- [ ] Custom icon sets

### Layout & Spacing
- [x] Base unit (4px)
- [x] Spacing scales
  - [x] Zero (0px)
  - [x] Extra Extra Small (4px)
  - [x] Extra Small (8px)
  - [x] Small (12px)
  - [x] Medium (16px)
  - [x] Large (20px)
  - [x] Extra Large (32px)
  - [x] Extra Extra Large (40px)

### Border Radius
- [x] Zero (0)
- [x] Small (8)
- [x] Medium (12)
- [x] Large (20)

### Atoms
- [x] Buttons
  - [x] Primary
  - [x] Secondary
  - [x] Ghost
  - [x] Icon buttons
- [x] Toggles
  - [x] Switch
  - [x] Radio
  - [x] Checkbox
- [ ] Avatar
  - [ ] User avatars
  - [ ] Placeholder avatars
- [x] Tags
  - [x] Status tags
  - [x] Label tags
- [ ] Duration Button
- [ ] Charts
- [x] Background styles

### Brand Assets
- [ ] Bipa Symbol
- [ ] Bipa Logo Horizontal
- [ ] Bipa Vertical
- [ ] Bipa Premium variants
- [ ] Logo guidelines
